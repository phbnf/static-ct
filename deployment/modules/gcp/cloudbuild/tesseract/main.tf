  ...
  build {
    ...
    dynamic "step" {
      ...
      content {
        ...
        wait_for = tg_path.key > 0 ? ["docker_push_conformance_gcp", "terraform_apply_conformance_staging_${tg_path.key}"] : ["docker_push_conformance_gcp"]
      }
    }
